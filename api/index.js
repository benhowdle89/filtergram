...
const { sendForgotPasswordEmail } = require("./../etc/send-mail");
...
api.post(
  ...
  asyncMiddleware(async (req, res) => {
    ...
    if (!token) {
      token = await model.createUserPasswordResetToken(user.id);
    }
    try {
...
api.post(
  ...
  asyncMiddleware(async (req, res) => {
    ...
    email = email.toLowerCase().trim();
    password = password.trim();
    const users = await model.getUserByEmailAndToken(email, token);
    if (!users.length) {
      return res.sendStatus(404);
    }
    const user = users[0];
    ...
    try {
      await model.updateUserPassword(user.id, hashedPassword);
    } catch (e) {
